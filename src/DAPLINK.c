...
ErrorCode_t DAP_EpInOut_Hdlr_Callback (USBD_HANDLE_T hUsb, void* data, uint32_t event) {
	...
	switch (event) {
	case USB_EVT_IN:
		if (daplink->usb_ins_start_idx == daplink->usb_ins_end_idx) {
			daplink->usb_ins_busy = 0;
			break;
		}
		USBD_API->hw->WriteEP(hUsb, pHidCtrl->epin_adr, daplink->usb_ins[daplink->usb_ins_start_idx], DAP_PACKET_SIZE);
		daplink->usb_ins_start_idx = (daplink->usb_ins_start_idx+1) % DAP_PACKET_COUNT;
		//LED_RUNNING_OUT (1);
		//LED_CONNECTED_OUT (0);
		break;
	case USB_EVT_OUT:
		USBD_API->hw->ReadEP(hUsb, pHidCtrl->epout_adr, daplink->usb_outs[daplink->usb_outs_end_idx]);
		daplink->usb_outs_end_idx = (daplink->usb_outs_end_idx+1) % DAP_PACKET_COUNT;
		//LED_CONNECTED_OUT (1);
		//LED_RUNNING_OUT (0);
		break;
	}
...
int main(void) {
    ...
    if (init_usb_hid (&usb_param,
			...
			(uint8_t**)&daplink, sizeof (DAPLINK_HND_T))) {
    	// printf error;
    	// LED error;
    	error_code (1);
    	return -1;
    ...
    while (1) {
    	...
    	sleep_ms (1000);
    	*/
    	if (daplink->usb_outs_start_idx == daplink->usb_outs_end_idx) {
    		//LED_ERROR_ON ();
    		__WFI ();
    		continue;
    	} else {
    	...
    	//LED_ERROR_OFF ();
    	DAP_ProcessCommand (daplink->usb_outs[daplink->usb_outs_start_idx], daplink->usb_ins[daplink->usb_ins_end_idx]);
    	daplink->usb_outs_start_idx = (daplink->usb_outs_start_idx+1) % DAP_PACKET_COUNT;
    	daplink->usb_ins_end_idx = (daplink->usb_ins_end_idx+1) % DAP_PACKET_COUNT;
    	if (!daplink->usb_ins_busy) { // kickstart
    		daplink->usb_ins_busy = 1;
    		uint8_t idx = daplink->usb_ins_start_idx;
    		daplink->usb_ins_start_idx = (daplink->usb_ins_start_idx+1) % DAP_PACKET_COUNT;
    		USBD_API->hw->WriteEP(g_usb_hnd, HID_EP_IN, daplink->usb_ins[idx], DAP_PACKET_SIZE);
